...
void Renderer::set_alpha(RenderPart part, guint16 alpha)
{
  pango_renderer_set_alpha(gobj(), ((PangoRenderPart)(part)), alpha);
}

guint16 Renderer::get_alpha(RenderPart part) const
{
  return pango_renderer_get_alpha(const_cast<PangoRenderer*>(gobj()), ((PangoRenderPart)(part)));
}
...
{
  Glib::RefPtr<Layout> retvalue = Glib::wrap(pango_renderer_get_layout(gobj()));
  if(retvalue)
...
{
  Glib::RefPtr<LayoutLine> retvalue = Glib::wrap(pango_renderer_get_layout_line(gobj()));
  if(retvalue)
    retvalue->reference(); //The function does not do a ref for us.
  return retvalue;
}
